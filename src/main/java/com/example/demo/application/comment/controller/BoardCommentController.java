...
import com.example.demo.application.comment.api.AbstractBoardCommentApi;
...
public class BoardCommentController extends AbstractBoardCommentApi {
    ...
    public ResponseEntity<ResponseBody<GlobalPageResponse<MyCommentResponse>>> getUserComments(
        ...
        Page<MyCommentInfo> myCommentInfoPage = commentService.getCommentsByUserId(userId, pageable, EntireBoardType.fromBoardType(boardType));
    ...
    public ResponseEntity<ResponseBody<CommentInfoResponse>> postComment(
        ...
        return ResponseEntity.ok(createSuccessResponse(CommentInfoResponse.from(commentService.postComment(commentRequest.toDomainForPost(userId, boardId), NotificationType.BOARD_COMMENT))));
    }
