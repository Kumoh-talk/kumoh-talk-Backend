...
public class CustomOAuth2UserServiceTest {
    ...
    class GetGithubLoginUserInformation {
        private String ENDPOINT_URI = "https://api.github.com/user";

        // 모의 OAuth 서버의 클라이언트 정보 및 발급하는 액세스 토큰 설정
        private ClientRegistration clientRegistration = createGithubClientRegistration();
        private OAuth2AccessToken accessToken = createAccessToken();
        // 모의 OAuth 서버에 보낼 요청 설정
        private OAuth2UserRequest userRequest = new OAuth2UserRequest(clientRegistration, accessToken);

        private final long USER_INFO_ID = 123456789L;
        private final String MOCK_RESPONSE = String.format("{"
                + "\"login\": \"octocat\","
                + "\"id\": %d,"
                + "\"node_id\": \"MDQ6VXNlcjE=\",\n"
        ...
        void 성공_사용자_정보를_얻을_수_있다() {
            ...
            mockServer
                    ...
                    .expect(requestTo(ENDPOINT_URI))
                    .andExpect(method(HttpMethod.GET))
                    // 해당 HTTP 메서드로 해당 URL에 요청이 오면 HTTP 상태 코드 200과 함께 모의 응답을 반환
                    .andRespond(withSuccess(MOCK_RESPONSE, MediaType.APPLICATION_JSON));
            ...
            assertSoftly(softly -> {
                ...
                softly.assertThat(userInfo.getId()).isEqualTo(String.valueOf(USER_INFO_ID));
    ...
    class GetGoogleLoginUserInformation {
        private final String ENDPOINT_URI = "https://www.googleapis.com/oauth2/v3/userinfo";

        // 모의 OAuth 서버의 클라이언트 정보 및 발급하는 액세스 토큰 설정
        private ClientRegistration clientRegistration = createGoogleClientRegistration();
        private OAuth2AccessToken accessToken = createAccessToken();
        // 모의 OAuth 서버에 보낼 요청 설정
        private OAuth2UserRequest userRequest = new OAuth2UserRequest(clientRegistration, accessToken);

        private final String USER_INFO_ID = "123456789012345678901";
        private final String MOCK_RESPONSE = String.format("{"
                + "\"sub\": \"%s\","
                + "\"name\": \"Kim Kumoh\","
                + "\"given_name\": \"Kumoh\","
        ...
        void 성공_사용자_정보를_얻을_수_있다() {
            ...
            assertSoftly(softly -> {
                ...
                softly.assertThat(userInfo.getId()).isEqualTo(USER_INFO_ID);
    ...
    class GetKakaoLoginUserInformation {
        private final String ENDPOINT_URI = "https://kapi.kakao.com/v2/user/me";

        // 모의 OAuth 서버의 클라이언트 정보 및 발급하는 액세스 토큰 설정
        private ClientRegistration clientRegistration = createKakaoClientRegistration();
        private OAuth2AccessToken accessToken = createAccessToken();
        // 모의 OAuth 서버에 보낼 요청 설정
        private OAuth2UserRequest userRequest = new OAuth2UserRequest(clientRegistration, accessToken);

        private final long USER_INFO_ID = 9876543210L;
        private final String MOCK_RESPONSE = String.format("{"
                + "\"id\": %d,"
    ...
    class GetNaverLoginUserInformation {
        private final String ENDPOINT_URI = "https://openapi.naver.com/v1/nid/me";

        // 모의 OAuth 서버의 클라이언트 정보 및 발급하는 액세스 토큰 설정
        private ClientRegistration clientRegistration = createNaverClientRegistration();
        private OAuth2AccessToken accessToken = createAccessToken();
        // 모의 OAuth 서버에 보낼 요청 설정
        private OAuth2UserRequest userRequest = new OAuth2UserRequest(clientRegistration, accessToken);

        private final String USER_INFO_ID = "12345678";
        private final String MOCK_RESPONSE = String.format("{"
                + "  \"resultcode\": \"00\","
                + "  \"message\": \"success\","
                + "  \"response\": {"
